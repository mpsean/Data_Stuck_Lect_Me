...
public class BST {
	...
	public void breadthFirstTraversal(BSTNode node) {
		QueueArray thisLevel = new QueueArray();
		QueueArray nextLevel = new QueueArray();
		QueueArray Ans = new QueueArray();
		BSTNode n = node;
		BSTNode print = n;
		try {
			...
			while(n!=null) {
				...
				n = (BSTNode) thisLevel.removeFirst();
				if(n.left!= null) {
					nextLevel.insertLast(n.left);
				}
				if(n.right!= null) {
					nextLevel.insertLast(n.right);
				}
				Ans.insertLast(n);
				
				//move nextLevel to thisLevel
				if(thisLevel.isEmpty()) {
					thisLevel = nextLevel;
					nextLevel = new QueueArray();
				}
			}
			int size = Ans.size();
			for(int i = 0; i<size; i++) {
				print = (BSTNode) Ans.removeFirst();
